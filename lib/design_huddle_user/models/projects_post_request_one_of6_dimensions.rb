...
module DesignHuddleUser
  class ProjectsPOSTRequestOneOf6Dimensions
    attr_accessor :width
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DesignHuddleUser::ProjectsPOSTRequestOneOf6Dimensions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DesignHuddleUser::ProjectsPOSTRequestOneOf6Dimensions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def _deserialize(type, value)
      ...
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        ...
        klass = DesignHuddleUser.const_get(type)
